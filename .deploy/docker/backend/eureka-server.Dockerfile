...
COPY backend/services/eureka-server/pom.xml ./services/eureka-server/
...
RUN mvn -f ./services/eureka-server/pom.xml dependency:go-offline -B -Dmaven.wagon.http.ssl.insecure=true -Dmaven.wagon.http.ssl.allowall=true 2>/dev/null || true

COPY backend/frameworks/zqzl-framework ./frameworks/zqzl-framework
COPY backend/services/eureka-server ./services/eureka-server
...
RUN mvn -f ./services/eureka-server/pom.xml clean package -DskipTests -Dmaven.wagon.http.ssl.insecure=true -Dmaven.wagon.http.ssl.allowall=true

FROM eclipse-temurin:11-jre
WORKDIR /app

COPY --from=builder /app/services/eureka-server/target/eureka-server-1.0.0.jar app.jar
